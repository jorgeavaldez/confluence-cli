...
class ConfluenceClient {
    ...
    if (pageIdOrUrl.includes(this.domain)) {
      ...
      if (displayMatch) {
        ...
        try {
          ...
          console.error('Error resolving page ID from display URL:', error.message);
        }
  ...
  async downloadAttachment(pageIdOrUrl, attachmentIdOrAttachment, options = {}) {
    ...
    const resolvedDownloadUrl = this.toAbsoluteUrl(downloadUrl);
    if (!resolvedDownloadUrl) {
      throw new Error('Unable to determine download URL for attachment');
    }

    const confluenceOrigin = this.getConfluenceOrigin();
    const confluenceHost = new URL(confluenceOrigin).host;
    const includeAuthHeader = this.isTrustedConfluenceUrl(resolvedDownloadUrl);

    // Download directly using axios.
    // Only include Authorization on trusted Confluence URLs and strip it if redirects leave that host.
    const downloadResponse = await axios.get(resolvedDownloadUrl, {
      ...
      beforeRedirect: (redirectOptions) => {
        const redirectHost = redirectOptions.host
          || `${redirectOptions.hostname || ''}${redirectOptions.port ? `:${redirectOptions.port}` : ''}`;
        const redirectProtocol = redirectOptions.protocol || 'https:';
        const isTrustedRedirect = redirectProtocol === 'https:' && redirectHost === confluenceHost;

        if (!isTrustedRedirect && redirectOptions.headers) {
  ...
  isTrustedConfluenceUrl(pathOrUrl) {
    if (!pathOrUrl) {
      return false;
    }

    try {
      const configuredOrigin = this.getConfluenceOrigin();
      const configured = new URL(configuredOrigin);
      const parsed = new URL(pathOrUrl, configuredOrigin);
      return parsed.protocol === 'https:' && parsed.host === configured.host;
    } catch {
      return false;
    }
  }
